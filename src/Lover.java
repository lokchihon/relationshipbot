...
public class Lover {
	...
	public String getResponse(String statement) {
		...
		if (statement.length() == 0) {
			response = "babe?";
		} else if (findKeyword(statement, "bad") >= 0 || findKeyword(statement, "terrible") >= 0
				|| findKeyword(statement, "hated it") >= 0) {
			response = "Why? ";
		} else if (findKeyword(statement, "good") >= 0 || findKeyword(statement, "amazing") >= 0
			...
			response = "Speaking of family, my parents misses" + " you and wants you to come over for dinner.";
		} else if (findKeyword(statement, "on a date") >= 0) {
			response = "Lets go on a date next week";
		} else if (findKeyword(statement, "I want to", 0) >= 0) {
			response = transformIWantToStatement(statement) + "Lets do that for our date.";
		} else if (findKeyword(statement, "I want", 0) >= 0) {
			response = transformIWantStatement(statement);
		} else if (findKeyword(statement, "to go on", 0) >= 0 || findKeyword(statement, "to go to", 0) >= 0
				|| findKeyword(statement, "hang out at", 0) >= 0 || findKeyword(statement, "to go in", 0) >= 0) {
	...
	private String transformYouMeStatement(String statement) {
		...
		return "Babe, what would make you think that I would" + restOfStatement + " you?";
	}
	...
	private String[] randomResponses = { "It's ok, I still love you", "Can you say that again, Honey?",
			"Do you really think so?", "Mom and dad are making fried chicken tonight. Come over", "Oh really?",
			"Do you want to go on a date this weekend?", "So, would you like to go see a movie?", "I love you",
			"<3 <3 <3" };
